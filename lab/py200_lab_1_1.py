...
class LinkedList:
    def __init__(self, nodes=None):
        if nodes is None:
            self.head = None
            self.tail = None
        elif isinstance(nodes, Node): # Один нод - можно только типы проверять. Это будет значить, что тут один нод был передан
    ...
    def insert(self, node, index=0):
        ...
        elif index > 0: # пока не понятно, как ограничить возможный индекс длиной Linkedlist + нужна проверка на int
            #TODO нужно реализовать __len__ в Linkedlist
            i = 0 # счетчик
...
l2.append_left(a5)
l2.append(a6)
print(l2.tail.next_.next_.data)
l2.insert(a7, 2)
print(l2.head.next_.next_.next_.data)
